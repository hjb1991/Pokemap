package net.luna.poketools.views;
...
import net.luna.poketools.controllers.service.PokemonNotificationService;
...
import net.luna.poketools.views.map.MapWrapperFragment;
import net.luna.poketools.views.settings.SettingsActivity;
...
public class MainActivity extends BaseActivity {
    ...
    private String username = "";//使用自己的pokemon go 账号
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nianticManager.login(username, password, new NianticManager.LoginListener() {
            @Override
            public void authSuccessful(String authToken) {
                LunaLog.d("Login successful");
                MainActivity.hasLogin = true;
            }

            @Override
            public void authFailed(String message) {
                LunaLog.d("Login failed");
            }
        });
    ...
    public void onEvent(SearchInPosition event) {
        if (event != null) {
            if (!StringUtils.isBlank(event.getName())) {
                nianticManager.findTargetPokemonFromThridApi(event.getPosition().latitude, event.getPosition().longitude, event.getName(), -1);
                return;
            }
            ...
            //精确查找
            if (hasLogin && pref.getUseOwnApi()) {
                for (LatLng p : list) {
                    nianticManager.getCatchablePokemon(p.latitude, p.longitude, 0D);
                }
            } else {
                nianticManager.getCatchablePokemonFromThridApi(event.getPosition().latitude, event.getPosition().longitude);
            }
        }
    }
