...
public class MarkerRefreshController {
    ...
    public void postMarker(PokemonMarkerExtended markerData) {
        long time = 0l;
        if (markerData.getCatchablePokemon() != null) {
            time = markerData.getCatchablePokemon().getExpirationTimestampMs() - System.currentTimeMillis();
        } else {
            time = markerData.getApiPokemon().getExpire() - System.currentTimeMillis();
        }
        if (time > 0) {
